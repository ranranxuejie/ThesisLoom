...
def _should_print_full_llm_output() -> bool:
    value = str(os.getenv("THESISLOOM_SHOW_LLM_OUTPUT", "0")).strip().lower()
...
def _extract_content_parts_from_chunk_obj(obj: Any) -> list[str]:
    ...
    def _append(value: Any) -> None:
        if isinstance(value, str):
            ...
            if isinstance(value.get("text"), str):
                ...
                parts.append(value.get("content", ""))
    ...
    if isinstance(obj_type, str):
        ...
        if obj_type == "string":
            ...
            if isinstance(data_field, dict):
                _append(data_field.get("content"))
                _append(data_field.get("text"))
                _append(data_field.get("message"))
            else:
                _append(data_field)
    ...
    if isinstance(choices, list):
        for choice in choices:
            ...
            _append(message.get("content"))

            _append(choice.get("text"))

            delta = choice.get("delta", {}) if isinstance(choice.get("delta", {}), dict) else {}
            _append(delta.get("content"))

    _append(obj.get("output_text"))
    _append(obj.get("content"))
    _append(obj.get("text"))

    response_obj = obj.get("response", {}) if isinstance(obj.get("response", {}), dict) else {}
    _append(response_obj.get("output_text"))
    return parts


def _sanitize_sse_leakage_text(text: Any) -> str:
    raw = str(text or "")
    if not raw:
        return ""

    # 常见污染：孤立的 data: [DONE]
    if raw.strip().lower() == "data: [done]":
        return ""

    lines = raw.splitlines()
    has_sse_line = any(line.lstrip("\ufeff \t").startswith("data:") for line in lines)
    if not has_sse_line:
        return raw.replace("data: [DONE]", "").strip()
    ...
    for line in lines:
        ...
        try:
            ...
            continue
        recovered_parts.extend(_extract_content_parts_from_chunk_obj(obj))

    if recovered_parts:
        return "".join(recovered_parts).strip()

    # 若无法恢复，至少移除 SSE 包装行，避免污染正文。
    cleaned = []
    for line in lines:
        normalized = line.lstrip("\ufeff \t")
        if normalized.lower().startswith("data: [done]"):
            continue
...
def call_llm(system_input: str,
    ...
    if 'doubao' in model:
        ...
        if user_input:
            ...
            # 兼容某些代理：仅有 system 消息时会出现空响应，补一个空 user 消息触发正常生成。
            ...
            try:
                ...
                with requests.post(
                    ...
                    for line in resp.iter_lines():
                        ...
                        decoded_line = decoded_line.lstrip("\ufeff \t").strip()
                        if not decoded_line:
                            continue
                        ...
                        if raw_data.startswith("event:"):
                            continue

                        raw_payloads.append(raw_data)
                        try:
                            data_obj = json.loads(raw_data)
                        except json.JSONDecodeError:
                            lowered = raw_data.lower()
                            if lowered == "[done]" or lowered.startswith("data:"):
                                continue
                            if (not lowered.startswith("id:")) and (not lowered.startswith("event:")) and (not lowered.startswith(":")):
                ...
                if (not full_content) and (not raw_payloads):
                    ...
                    try:
                        ...
                        if text2:
                            full_content = _sanitize_sse_leakage_text(text2)
    ...
    response = _sanitize_sse_leakage_text(response)
    # 防止 SSE 结束标记混入最终输出
