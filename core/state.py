...
def load_topic_from_inputs_json() -> str:
    try:
        payload = json.load(open(resolve_inputs_path(), "r", encoding="utf-8-sig"))
...
def _safe_name(text: str) -> str:
    return ''.join(ch for ch in str(text) if ch not in '\\/*?:"<>|').replace(" ", "_")
...
def output_path_from_checkpoint(checkpoint_path: str) -> str:
    path = str(checkpoint_path or "")
    suffix = "_checkpoint.json"
    if path.endswith(suffix):
        return path[:-len(suffix)] + ".md"
    return ""


def migrate_paths_after_topic_update(
    model: str,
    topic: str,
    prompt_language: str,
    output_path: str,
    ...
    new_output_path, new_checkpoint_path = build_output_paths(model, topic, prompt_language)
    ...
    if output_path and os.path.exists(output_path) and (os.path.normpath(output_path) != os.path.normpath(new_output_path)):
        os.replace(output_path, new_output_path)

    return new_output_path, new_checkpoint_path
...
class PaperWriterState:
    ...
    def __init__(self, inputs: Dict[str, Any], input_from_md: bool = True):
        ...
        self.pre_done_title: bool = bool(inputs.get("pre_done_title", False))
        self.pre_done_query_builder: bool = bool(inputs.get("pre_done_query_builder", False))
        self.pre_done_paper_search: bool = bool(inputs.get("pre_done_paper_search", False))
        self.pre_done_related_confirm: bool = bool(inputs.get("pre_done_related_confirm", False))
        self.pre_done_research_gaps: bool = bool(inputs.get("pre_done_research_gaps", False))
